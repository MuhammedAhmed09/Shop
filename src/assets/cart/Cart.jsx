          ...
          <div key={item.id} className='rounded shadow-sm border-b-blue-500 p-5 h-auto'>
            ...
            <div className='flex justify-between'>
              {/* <button onClick={() => dispatch(inCrement(item.id))}
                className='rounded-full border-2 px-4 font-semibold text-white bg-gray-800'>+
              </button>
              <p className='rounded-full border-2 px-4 border-slate-950'>{item.quantity}</p>
              <button
                onClick={() => dispatch(deCrement(item.id))}
                className='rounded-full text-white px-4 bg-red-600 font-semibold'
              >
                -
              </button> */}
            </div>
            ...
            {/* <button onClick={() => dispatch(Removeitem(item.id))} 
              className='rounded mt-4 p-1 w-full text-white bg-red-600 px-4'>Remove 
            </button> */}
          </div>
        ))}
      </div>
