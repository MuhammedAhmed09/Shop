...
const Header = () => {
  ...
  const product = useSelector((state) => state.rootReducer.product);
    ...
    <div className='ourNav header bg-transparent h-[55px] flex items-center px-[15%] justify-between p-2 '>
      ...
      <div>
        <ul id='Links' className='flex text-1xl items-center max-h-0 font-semibold'>
          ...
          <li className=' hover:bg-red-600 hover:text-white rounded '><Link to='/cart'>Cart<span className='ml-2'>{product.length >= 1 ? product.length : <span></span> }</span></Link></li>
